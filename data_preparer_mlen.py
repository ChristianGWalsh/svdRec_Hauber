...
class DataPreparer_movielen:
    #Changing this for now so it fits the million song data
    #def __init__(self, file_path, sep='\t', user_col='user_id', item_col='item_id', rating_col='rating', timestamp_col='timestamp'):
    def __init__(self, file_path, sep='\t', user_col='user_id', item_col='song_id', rating_col='plays'):
    ...
    def load_data(self):
        ...
        columns = [self.user_col, self.item_col, self.rating_col]
